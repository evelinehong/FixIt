    ...
    def forward(self, attr, state, Rr, Rs, Ra, Rr_idxs,n_particles, node_r_idx, node_s_idx, pstep, rels_types,
        ...
        for stage_id in range(self.n_stages):
            ...
            for s in rel_idx_of_stage:
                ...
                node_r = torch.from_numpy(node_r_idx[s].astype(np.float32)).cuda()
                node_s = torch.from_numpy(node_s_idx[s].astype(np.float32)).cuda()
                
                Rrp_global_idx = Rrp.mv(node_r)
                Rsp_global_idx = Rsp.mv(node_s)
                nrels = Rrp.shape[0]
